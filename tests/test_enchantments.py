...
class TestEnchantments(unittest.TestCase):
    ...
    def test_date_multiple_voting(self):
        ed = enchantments.EnchantedDateVoting(
            "yyyymmdd", "2010 8.8.1991 on August the 8th 1991")
        self.assertEqual(ed.val, (8, 8, 1991))

    def test_date_with_rng(self):
        # 2010 is in te given range, thus it wil prescede 8,8,1991
        ed = enchantments.EnchantedDate(
            "yyyymmdd", "2010 8.9.1991 - 2012 on August the 8th 1991")
    ...
    def test_error(self):
        ret = str(enchantments.enchant(
            "error", {'symbol': 'sym', 'kw': dict(a=1, b=2, c='ha')}))
        self.assertEqual(ret, '(((error sym :a "1" :c "ha" :b "2")))')
